    ...
    <div class="default-page" ref="defaultPageRef">
      ...
      <div class="chat-send-footer" ref="footerSendRef">
        ...
        <button
          ...
          :class="btnDisabled ? 'disalbed-btn' : ''"
          @click="handleSubmit"
        ...
        <ul class="voiceList" v-for="(voiceItem, index) in voiceList">
          <li>
            ...
            <audio :src="voiceItem.voiceUrl" controls />
          </li>
        </ul>
      </div>
    </div>
...
import VoicingRuning from "./component/VoicingRuning.vue";
...
export default defineComponent({
  ...
  setup(this, props) {
    ...
    watchEffect(() => {
      const hasBotResponse = chatList.find(
        (chatItem) => chatItem?.userId === "assistant"
      );
      nextTick(() => {
        ...
        const distance =
          lastBotDom.getBoundingClientRect?.()?.top -
          firstEleDom?.getBoundingClientRect?.()?.top;
        scrollBox.scrollTo({
          top: distance,
        });
      });
  ...
  components: { ChatDetail, Modal, VoicingRuning },
});
</script>
