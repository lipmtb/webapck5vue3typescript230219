...
export const afterLeftRightMidTrace = (
  root: TreeNode | null,
  result: string[]
) => {
  if (!root) {
...
// 根据中序和后序返回先序
export const getFirstByMidAndAfterTree = (
  midStr: string,
  afterStr: string,
  result: string[]
) => {
...
// 根据中序和后序，并构建二叉树
export const getFirstByMidAndAfterBuildTree = (
  midStr: string,
  afterStr: string
) => {
  ...
  if (rightMidStr.length && rightAfterStr.length) {
    currentRoot.right = getFirstByMidAndAfterBuildTree(
      rightMidStr,
      rightAfterStr
    );
  }
...
export const getAfterByFirstMidTrace = (firstStr: string, midStr: string) => {
  ...
  if (leftFirstStr && leftMidStr) {
    currentRoot.left = getAfterByFirstMidTrace(
      leftFirstStr,
      leftMidStr
    ) as TreeNode;
  }
  ...
  if (rightFirstStr && rightMidStr) {
    currentRoot.right = getAfterByFirstMidTrace(
      rightFirstStr,
      rightMidStr
    ) as TreeNode;
  }
...
export const compareTwoTreeAndReturnBoolean: (
  rootA?: TreeNode | null,
  rootB?: TreeNode | null
) => boolean = (rootA?: TreeNode | null, rootB?: TreeNode | null) => {
  if (!rootA && !rootB) {
    return true;
  }
  if (rootA === rootB) {
  ...
  const rightResult = compareTwoTreeAndReturnBoolean(
    rootA?.right,
    rootB?.right
  );
